...
def audit_models():
    ...
    print(f"1. YOLO Model (yolo26n.pt): {'LOADED (OK)' if yolo_model else 'FAILED'}")
    if yolo_model:
        print(f"   Total detectable classes: {len(yolo_model.names)}")

    print(f"2. RapidOCR (ONNX Engine): {'AVAILABLE (OK)' if RAPIDOCR_AVAILABLE else 'UNAVAILABLE'}")
    print(f"3. Tesseract Engine: {'AVAILABLE (OK)' if TESSERACT_AVAILABLE else 'UNAVAILABLE (Optional)'}")
    ...
    for img_p in images:
        ...
        if dets:
            for d in dets:
                print(f"  -> Detected: {d['product']} | Class: {d['raw_class']} | Conf: {d['confidence']*100:.1f}% | Box: {d['boundingBox']}")
        else:
            print("  -> No direct COCO appliance detected (delegates to Qwen/heuristic fallback)")
